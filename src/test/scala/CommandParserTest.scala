...
class CommandParserStressTest extends AnyFlatSpec with HasCliOptions with Cli.EmitVcd with ChiselSim {
  // Change n here to run the full suite at a different parallelism.
  // All matrix dimensions are expressed as multiples of n, so every test
  // stays valid regardless of the value chosen.
  val n = 15

  // ─── Memory depth helpers ───────────────────────────────────────────────────
  // Scale memory sizes with n so they are always large enough.
  // The weight memory must hold (M/n)*(K/n)*n words per layer.
  // For the largest test (12×12 weight matrix) that is 3*3*n = 9n words.
  // Using 4096*n/4 gives comfortable headroom for any n.
  val wtMemDepth   = math.max(4096, 64 * n)
  val actMemDepth  = math.max(4096, 64 * n)
  val outMemDepth  = math.max(4096, 64 * n)
  val biasMemDepth = math.max(256,  16 * n)

  // bufferBBase must be placed beyond the activation input.
  // For a layer with K rows and N columns the activation footprint is
  // (K/n)*(N/n)*n words.  We add a generous guard band.
  ...
  def readOutput(dut: FullSystemHarness, m: Int, nn: Int, baseAddr: Int = 0): Array[Array[Long]] = {
    ...
    for (tR <- 0 until m / n; tC <- 0 until tilesN; r <- 0 until n) {
      // Each output element is 4 bytes; we read n elements per row-within-tile
  ...
  // ─── Convenience runner for a complete single-layer inference ────────────────
  ...
  def runSingleLayer(dut: FullSystemHarness, W: Array[Array[Int]], A: Array[Array[Int]],
    ...
    val bufBBase = safeBufferBBase(k, nn)
  ...
  //
  // All matrix dimensions are written as multiples of n so the suite is valid
  // for any power-of-two n.  A comment after each dimension shows the concrete
  // value when n=4 to make the intent easy to read.

  "CommandParser Stress" should "handle identity n×n" in {
    simulate(new FullSystemHarness(n)) { dut =>
      dut.io.rx_valid.poke(false.B); dut.io.tx_ready.poke(true.B); waitForInit(dut)
      val m = n; val k = n; val nn = n          // 4×4×4 when n=4
      ...
      assertResult(result, expected, m, nn, s"identity ${n}x${n}")
    }
  }

  it should "handle identity 2n×2n tiled" in {
    simulate(new FullSystemHarness(n)) { dut =>
      dut.io.rx_valid.poke(false.B); dut.io.tx_ready.poke(true.B); waitForInit(dut)
      val m = 2*n; val k = 2*n; val nn = 2*n   // 8×8×8 when n=4
      val W        = Array.tabulate(m, k)((r, c) => if (r == c) 1 else 0)
      val A        = Array.tabulate(k, nn)((r, c) => r * nn + c + 1)
      val bias     = Array.fill(nn)(0)
      val expected = expectedSingleLayer(W, A, bias, m, k, nn, 0, 0, 6, false)
      val result   = runSingleLayer(dut, W, A, bias, m, k, nn, 0, 0, 6, false)
      assertResult(result, expected, m, nn, s"identity ${2*n}x${2*n}")
    }
  }
  ...
  it should "handle random 2n×2n tiled no clamp" in {
    simulate(new FullSystemHarness(n)) { dut =>
      dut.io.rx_valid.poke(false.B); dut.io.tx_ready.poke(true.B); waitForInit(dut)
      ...
      assertResult(result, expected, m, nn, s"random ${2*n}x${2*n} relu no clamp")
    }
  }

  it should "handle non-square 2n×n × n×2n no clamp" in {
    simulate(new FullSystemHarness(n)) { dut =>
      dut.io.rx_valid.poke(false.B); dut.io.tx_ready.poke(true.B); waitForInit(dut)
      val rng = new Random(300)
      val m = 2*n; val k = n; val nn = 2*n      // 8×4×8 when n=4
      val W        = Array.fill(m, k)(rng.nextInt(256) - 128)
      val A        = Array.fill(k, nn)(rng.nextInt(256) - 128)
      ...
      assertResult(result, expected, m, nn, s"nonsquare ${2*n}x${n}*${n}x${2*n} no clamp")
    }
  }

  it should "handle random n×n with clamp" in {
    simulate(new FullSystemHarness(n)) { dut =>
      dut.io.rx_valid.poke(false.B); dut.io.tx_ready.poke(true.B); waitForInit(dut)
      ...
      assertResult(result, expected, m, nn, s"random ${n}x${n} clamp")
    }
  }

  it should "handle random 3n×3n tiled" in {
    simulate(new FullSystemHarness(n)) { dut =>
      dut.io.rx_valid.poke(false.B); dut.io.tx_ready.poke(true.B); waitForInit(dut)
      val rng = new Random(380)
      val m = 3*n; val k = 3*n; val nn = 3*n   // 12×12×12 when n=4
      val W        = Array.fill(m, k)(rng.nextInt(256) - 128)
      val A        = Array.fill(k, nn)(rng.nextInt(256) - 128)
      val bias     = Array.fill(nn)(rng.nextInt(500) - 250)
      val expected = expectedSingleLayer(W, A, bias, m, k, nn, 2, 6, 6, false)
      val result   = runSingleLayer(dut, W, A, bias, m, k, nn, 2, 6, 6, false)
      assertResult(result, expected, m, nn, s"random ${3*n}x${3*n} leaky relu")
    }
  }
  ...
  it should "handle two-layer inference" in {
    simulate(new FullSystemHarness(n)) { dut =>
      ...
      val m0 = n;  val k0 = n;  val n0 = n     // all n×n×n when n=4
      ...
      val b0Base = 0
      val b1Base = n0 / n
      val bufBBase = safeBufferBBase(k0, n0)
  ...
  it should "handle three-layer inference" in {
    simulate(new FullSystemHarness(n)) { dut =>
      ...
      // All layers are n×n×n (4×4×4 when n=4)
      val sizes  = Seq((n, n, n), (n, n, n), (n, n, n))
      val funcs  = Seq(1, 2, 0)
      val shifts = Seq(3, 3, 0)
      val clamps = Seq(true, true, false)

      val bufBBase = safeBufferBBase(sizes(0)._2, sizes(0)._3)
  ...
  it should "handle deep K accumulation n×3n × 3n×n" in {
    simulate(new FullSystemHarness(n)) { dut =>
      dut.io.rx_valid.poke(false.B); dut.io.tx_ready.poke(true.B); waitForInit(dut)
      val rng = new Random(700)
      val m = n; val k = 3*n; val nn = n        // 4×12×4 when n=4
      ...
      assertResult(result, expected, m, nn, s"deep K ${n}x${3*n}*${3*n}x${n}")
    }
  }
  ...
  it should "handle large bias values" in {
    simulate(new FullSystemHarness(n)) { dut =>
      ...
      val W    = Array.tabulate(m, k)((r, c) => if (r == c) 1 else 0)
      val A    = Array.fill(k, nn)(0)
      // Cycle through four interesting bias values, tiling to length nn
      val biasPattern = Array(10000, -10000, 32767, -32768)
      val bias = Array.tabulate(nn)(i => biasPattern(i % biasPattern.length))
      val expected = expectedSingleLayer(W, A, bias, m, k, nn, 0, 0, 6, false)
      val result   = runSingleLayer(dut, W, A, bias, m, k, nn, 0, 0, 6, false)
